...
/**
 * Kryo {@link Serializer}s for lists created via {@link List#subList(int, int)}. An instance of a
 * serializer can be obtained via {@link #createFor(Class)}, which just returns <code>null</code> if
 * the given type is not supported by these serializers.
 *
...
public class SubListSerializers {
  ...
  /**
   * Obtain a serializer for the given sublist type. If the type is not supported <code>null</code>
   * is returned.
   *
  ...
  public static Serializer<List<?>> createFor(final Class type) {
    if (ArrayListSubListSerializer.canSerialize(type)) return new ArrayListSubListSerializer();
    if (JavaUtilSubListSerializer.canSerialize(type)) return new JavaUtilSubListSerializer();
    return null;
  }
  ...
  private static class SubListSerializer extends Serializer<List<?>> {
    ...
    private static Field getOffsetField(Class<?> clazz) throws NoSuchFieldException {
      try {
        // up to jdk8 (which also has an "offset" field (we don't need) - therefore we check
        // "parentOffset" first
  ...
  public static class ArrayListSubListSerializer extends Serializer<List<?>> {
    ...
    public static final Class<?> SUBLIST_CLASS =
        SubListSerializers.getClassOrNull("java.util.ArrayList$SubList");
     ...
     * @return <code>true</code> if the given class can be serialized/deserialized by this
     *     serializer.
     */
  ...
  public static class AbstractListSubListSerializer extends Serializer<List<?>> {
    ...
    public static final Class<?> SUBLIST_CLASS =
        SubListSerializers.getClassOrNull("java.util.AbstractList$SubList");

    private final SubListSerializer delegate =
        new SubListSerializer("java.util.AbstractList$SubList");
  ...
  /**
   * Supports sublists created via {@link AbstractList#subList(int, int)}, e.g. LinkedList. In
   * oracle jdk such sublists are represented by <code>java.util.SubList</code>.
   */
  public static class JavaUtilSubListSerializer extends Serializer<List<?>> {

    public static final Class<?> SUBLIST_CLASS =
        SubListSerializers.getClassOrNull("java.util.SubList");
