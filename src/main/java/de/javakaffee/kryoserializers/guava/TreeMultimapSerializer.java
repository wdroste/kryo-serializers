...
/**
 * A kryo {@link Serializer} for guava-libraries {@link TreeMultimap}. For reading / writing, the
 * default comparator is assumed so the multimaps are not null-safe. For copying, the copy contains
 * the same comparator instances as the original.
 */
public class TreeMultimapSerializer
  ...
  @Override
  public TreeMultimap<Comparable, Comparable> read(
      Kryo kryo, Input input, Class<? extends TreeMultimap<Comparable, Comparable>> type) {
    Comparator<? super Comparable> keyComparator =
        (Comparator<? super Comparable>) kryo.readClassAndObject(input);
    Comparator<? super Comparable> valueComparator =
        (Comparator<? super Comparable>) kryo.readClassAndObject(input);
    final TreeMultimap<Comparable, Comparable> multimap =
