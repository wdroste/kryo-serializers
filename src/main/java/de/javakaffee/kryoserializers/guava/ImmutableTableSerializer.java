...
public class ImmutableTableSerializer<R, C, V>
  ...
  @Override
  public ImmutableTable<R, C, V> read(
      Kryo kryo, Input input, Class<? extends ImmutableTable<R, C, V>> type) {
    final ImmutableTable.Builder<R, C, V> builder = ImmutableTable.builder();
    super.readTable(
        kryo,
        input,
        new CellConsumer<R, C, V>() {
          @Override
  ...
  public static void registerSerializers(final Kryo kryo) {
    ...
    kryo.register(
        ImmutableTable.builder().put("a", 1, 1).put("b", 1, 1).build().getClass(),
        serializer); // DenseImmutableTable
  }
}
