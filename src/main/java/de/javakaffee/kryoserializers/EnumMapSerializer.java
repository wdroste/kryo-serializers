...
public class EnumMapSerializer extends Serializer<EnumMap<? extends Enum<?>, ?>> {
  ...
  static {
    try {
      ...
      throw new RuntimeException(
          "The EnumMap class seems to have changed, could not access expected field.", e);
    }
  }
  ...
  public EnumMap<? extends Enum<?>, ?> copy(
      final Kryo kryo, final EnumMap<? extends Enum<?>, ?> original) {
    // Make a shallow copy to copy the private key type of the original map without using
    // reflection.
  ...
  @SuppressWarnings({"unchecked", "rawtypes"})
  private EnumMap<? extends Enum<?>, ?> create(
      final Kryo kryo,
      final Input input,
  ...
  public EnumMap<? extends Enum<?>, ?> read(
      final Kryo kryo,
      final Input input,
      final Class<? extends EnumMap<? extends Enum<?>, ?>> type) {
