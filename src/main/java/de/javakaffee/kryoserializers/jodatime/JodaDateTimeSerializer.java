...
/**
 * A format for joda {@link DateTime}, that stores the millis, chronology and time zone as separate
 * attributes. If the chronology is {@link ISOChronology}, the attribute is omitted, thus {@link
 * ISOChronology} is seen as default. If the time zone is the default time zone ({@link
 * DateTimeZone#getDefault()}), the time zone attribute is omitted. This requires different machines
 * to have the same time zone settings.
 *
...
public class JodaDateTimeSerializer extends Serializer<DateTime> {
  ...
  private DateTimeZone readTimeZone(final Input input) {
    ...
    // special case for "" to maintain backwards compatibility, but generally this is considered
    // harmful,
