...
/**
 * A more efficient kryo {@link Serializer} for {@link GregorianCalendar} instances (which are
 * created via <code>Calendar.getInstance()</code> if the locale is not thai or japanese, so
 * JapaneseImperialCalendar and BuddhistCalendar are not supported by this serializer).
 *
 * <p>With the default reflection based serialization, a calendar instance (created via <code>
 * Calendar.getInstance(Locale.ENGLISH)</code>) would take 1323 byte, this one only takes 24 byte.
 *
...
public class GregorianCalendarSerializer extends Serializer<GregorianCalendar> {
  ...
  @Override
  public GregorianCalendar read(
      final Kryo kryo, final Input input, final Class<? extends GregorianCalendar> type) {
